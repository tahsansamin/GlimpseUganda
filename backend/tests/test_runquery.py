...
def test_run_query_returns_llm_answer():
    ...
    with patch("app_main.PineconeVectorStore") as mock_vs, \
        ...
        result = run_query("kampala", "What is Kampala like?")

    assert result == "Kampala is a vibrant city."

def test_run_query_no_reranked_docs_returns_fallback():
    ...
    assert result == "I could not find relevant information for your question."
...
def test_run_query_context_passed_to_llm():
    ...
    with patch("app_main.PineconeVectorStore") as mock_vs, \
        ...
        from app_main import run_query
        run_query("kampala", "What markets does Kampala have?")
